...
---
C4-PlantUML 结合了 PlantUML 和 C4 模型的优点，通过使用开源和平台独立工具的直观语言，提供了一种描述和交流软件架构的简单方法——尤其是在前期设计会议期间。 
C4-PlantUML 包括用于使用 PlantUML 创建 C4 图的宏、构造型和其他好东西（如 VSCode 片段）。

{% include toc %}

> 原文地址: [plantuml-stdlib/C4-PlantUML](https://github.com/plantuml-stdlib/C4-PlantUML/edit/master/README.md)
...
在你的 C4 PlantUML `.puml` 文件的顶部，你需要在这个 repo 的 `root`目录中找到的 `C4_Context.puml`、`C4_Container.puml` 或 `C4_Component.puml` 文件。
要独立于任何 Internet 连接，您还可以将它下载下来,在 `root`目录中找到的文件，并使用附加命令行参数 `-DRELATIVE_INCLUDE="."`（包括本地文件）激活本地转换
...
(如果您不想在本地运行 PlantUML，您可以使用 e.g. the [PlantUML Web Server](https://www.plantuml.com/plantuml/uml/ZOxDIWGn48JlUOeufn5qSjcJfvNHsugBFsV99iqcsEc4T0VTjpSCE2AYUAeAgVwgjYosIakevytBBK824bPdaHms3pg85BuofjgtwHWbj4DZg2wJzDpaSZAliRh04ioykToZ9Nc-snbux_yUlEdGkOTj9AXJwJLAxQ5ofh4iSetHyeKUTlO0E7HpNoHcigXlW5sDosiuLojaT9_kn-aJk40Py_7q1-Znn09fv4N-swuU0ByFNbVyZlYQqmbR8DyIVW00).)

包含 C4_Container.puml 后，您可以使用 C4 元素定义的宏定义：“Person”、“Person_Ext”、“System”、“System_Ext”、“Container”、“Relationship”、“Boundary”和`系统_边界`
...
Entities 也可以使用 图标/sprites 装饰  使用 `$sprite` 参数, for example:
